...
from whatsapp.subscribe_flow import pending_screenshot, truncate
...
async def handle_inbound_image(phone: str, media_id: str, mime_type: str) -> None:
    """Process an inbound image. Currently only used for team screenshots."""
    if phone not in pending_screenshot:
        # Not in the screenshot onboarding state — silently ignore.
        # We don't have a use case for other inbound images yet.
    ...
    try:
        ...
        await send_message(phone, truncate(
            "Couldn't download that image. Try again, or text your 5 driver codes."
        ))
        return

    result = await extract_team_from_image(image_bytes, mime_type=detected_mime or mime_type)
    ...
    if fields:
        ...
        logger.bind(phone=mask_phone(phone)).info(
    ...
    pending_screenshot.discard(phone)
    ...
    # Partial: save what we have, ask for the missing pieces.
    await send_message(phone, (
        "Got most of it:\n\n"
        f"{_summary_line(result.team)}\n\n"
